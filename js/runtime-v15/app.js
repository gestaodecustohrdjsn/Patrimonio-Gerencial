  ...
  <dialog id="moveDialog" class="operation-dialog">
    <form method="dialog" id="moveForm">
      ...
      <div class="form-grid">
        ...
        <label>Movimentado por *<input id="moveBy" required maxlength="120" placeholder="Quem realizou a movimentação"></label>
        <label>Recebido por *<input id="moveReceivedBy" required maxlength="120" placeholder="Quem recebeu no destino"></label>
        <label class="full">Observação<textarea id="moveObservation" rows="2" placeholder="Observações da movimentação"></textarea></label>
      </div>
...
function mobilityBadge(asset) {
  const mobile = asset.mobilidade === "MOVEL";
  return `<span class="context-badge ${mobile ? "badge-mobile" : "badge-fixed"}">${mobile ? "Móvel" : "Fixo"}</span>`;
}

function maintenanceBadge(asset) {
  return asset.em_manutencao ? '<span class="context-badge badge-maintenance">Em manutenção</span>' : "";
}
  ...
  document.querySelector("#viewRoot").innerHTML = `
    ...
    ${state.detailEditMode && editAllowed ? detailEditFormHtml(asset) : `
      <section class="detail-grid">
        ${detailCard("Identificação", [["ID interna", asset.id_interna], ["ID SES", asset.id_ses || "—"], ["Tipo", `${typeName(asset.tipo_id)} · ${asset.mobilidade === "MOVEL" ? "Móvel" : "Fixo"}`], ["Centros permitidos", allowedCentersDisplay(asset)], ["Item de referência SIGEM", asset.descricoes_padrao?.descricao || "—"], ["Marca/Modelo", asset.marca_modelo || "—"]])}
        ...
        ${detailCard("Localização e condição", [["Centro de custo", asset.centros_custo?.nome || "—"], ["Estado de conservação", conservationName(asset.estado_conservacao)], ["Status", `${asset.removido ? "Removido" : (asset.status === "ATIVO" ? "Ativo" : "Inativo")}${asset.em_manutencao ? " · Em manutenção" : ""}`], ["Criado em", formatDateTime(asset.criado_em)]])}
      </section>`}
    ...
    <article class="maintenance-item">
      ...
      ${state.maintenanceCompletingId === m.id ? `<form class="finish-maintenance-form" data-maintenance-finish-id="${m.id}">
        <div class="form-grid">
          ...
          <label class="full">Observação da conclusão<textarea name="observacao_conclusao" rows="2"></textarea></label>
        </div>
