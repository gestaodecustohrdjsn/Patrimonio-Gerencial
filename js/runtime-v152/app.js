    ...
    <form method="dialog" id="moveForm">
      ...
      <div class="operation-layout">
        ...
        <section class="operation-section">
          ...
          <div class="form-grid operation-people-grid">
      ...
      <section id="moveMobilitySection" class="operation-subpanel" hidden>
        <div class="operation-subpanel-head"><div><strong>Configuração de mobilidade após a movimentação</strong><small>O destino passa a ser o centro atual e sai da lista de centros permitidos.</small></div></div>
        <label class="form-check form-switch"><input class="form-check-input" id="moveContinueMobile" type="checkbox"><span class="form-check-label">Continuar como patrimônio móvel</span></label>
        <div id="moveMobileOptions" hidden>
          <label class="form-check"><input class="form-check-input" id="moveAddOrigin" type="checkbox"><span class="form-check-label">Adicionar o centro de origem entre os centros permitidos</span></label>
          <div class="chip-picker">
            <label>Ajustar centros permitidos</label>
...
function renderMoveAllowedCenters() {
  ...
  if (!asset) return;
  const section = document.querySelector("#moveMobilitySection");
  const options = document.querySelector("#moveMobileOptions");
  const continuing = document.querySelector("#moveContinueMobile")?.checked;
  if (section) section.hidden = asset.mobilidade !== "MOVEL";
  if (options) options.hidden = !continuing;
  ...
  document.querySelector("#viewRoot").innerHTML = `
    ...
    ${state.detailEditMode && editAllowed ? detailEditFormHtml(asset) : `
      <section class="detail-grid">
        ...
        ${detailCard("Localização e condição", [["Centro de custo", asset.centros_custo?.nome || "—"], ...(asset.mobilidade === "MOVEL" ? [["Mobilidade", "Móvel"], ["Centros permitidos", allowedCentersDisplay(asset)]] : []), ["Estado de conservação", conservationName(asset.estado_conservacao)], ["Status", asset.removido ? "Removido" : (asset.status === "ATIVO" ? "Ativo" : "Inativo")], ...(asset.em_manutencao ? [["Condição operacional", "Em manutenção"]] : []), ["Criado em", formatDateTime(asset.criado_em)]])}
      </section>`}
...
function bindStaticActions() {
  ...
  document.querySelector("#moveContinueMobile")?.addEventListener("change", renderMoveAllowedCenters);
  document.querySelector("#moveAddOrigin")?.addEventListener("change", () => {
    const asset = selectedAsset();
    if (!asset) return;
    if (document.querySelector("#moveAddOrigin").checked) state.moveAllowedCenters.add(asset.centros_custo?.id);
    else state.moveAllowedCenters.delete(asset.centros_custo?.id);
    renderMoveAllowedCenters();
  });
  ...
  document.querySelector("#moveCenter")?.addEventListener("change", () => {
    const destination = document.querySelector("#moveCenter").value;
    if (destination) state.moveAllowedCenters.delete(destination);
    renderMoveAllowedCenters();
  });
}
...
function openMoveDialog() {
  ...
  const section = document.querySelector("#moveMobilitySection");
  section.hidden = asset.mobilidade !== "MOVEL";
  const continueMobile = document.querySelector("#moveContinueMobile");
  continueMobile.checked = asset.mobilidade === "MOVEL";
  document.querySelector("#moveAddOrigin").checked = false;
  renderMoveAllowedCenters();
...
async function submitMovement(event) {
  ...
  const continueMobile = asset.mobilidade === "MOVEL" && document.querySelector("#moveContinueMobile").checked;
  const payload = {
    ...
    mobilidade: continueMobile ? "MOVEL" : "FIXO",
    centrosPermitidosIds: continueMobile ? [...state.moveAllowedCenters].filter(id => id !== destination) : []
  };
      ...
      <section class="public-shell">
        ...
        <section class="public-hero">
          ...
          <div class="public-hero-status">
            ...
            ${asset.mobilidade === "MOVEL" ? '<span class="public-context-tag">[Móvel]</span>' : ""}
