...
function render() {
  ...
  switch (state.currentView) {
    ...
    case "centros":
      ...
      break;
    case "importacao":
      placeholder("Importação", "Receberá CSV, validação e de-para antes da confirmação.");
      break;
    case "etiquetas":
      placeholder("Etiquetas", "Será conectado ao layout de QR Code e PDF em lote.");
      break;
    case "relatorios":
      placeholder("Relatórios", "Exportações por filtros e seleção de colunas.");
      break;
    case "configuracoes": {
      const email = escapeHtml(state.session?.user?.email || "");
      placeholder("Configurações", `Usuário conectado: ${email}.`);
      break;
    }
    default:
      renderDashboard();
