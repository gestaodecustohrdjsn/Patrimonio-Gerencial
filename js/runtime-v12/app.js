  ...
  <dialog id="assetDialog">
    <form method="dialog" id="assetForm">
      ...
      <fieldset class="form-section"><legend>Identificação</legend><div class="form-grid">
        ...
        <label class="full sigem-field">Item de referência SIGEM <span class="optional-note">(Opcional)</span><input id="sigemSearch" autocomplete="off" placeholder="Pesquise um item de referência, se houver"><input id="descricaoPadraoId" type="hidden"><div id="sigemResults" class="search-results" hidden></div><small>O patrimônio pode ser cadastrado sem vínculo com o catálogo SIGEM.</small></label>
      </div></fieldset>
      ...
      <fieldset class="form-section"><legend>Aquisição e valores</legend><div class="form-grid">
        ...
        <label>Valor de aquisição <span class="optional-note">(Opcional)</span><input id="valor" inputmode="decimal" placeholder="R$ 0,00"></label>
...
function bindStaticActions() {
  ...
  document.querySelector("#viewRoot").addEventListener("click", (event) => {
    ...
    if (row?.dataset.assetId) openAssetDetail(row.dataset.assetId);
  });
  document.querySelector("#viewRoot").addEventListener("keydown", (event) => {
    const row = event.target.closest?.(".asset-row");
    if (row?.dataset.assetId && event.key === "Enter") openAssetDetail(row.dataset.assetId);
  });
}
