  ...
  const list = state.maintenance.length ? `<div class="maintenance-list">${state.maintenance.map(m=>`
    <article class="maintenance-item">
      ...
      <div class="maintenance-meta">
        ${m.solicitante ? `<span><strong>Solicitante:</strong> ${escapeHtml(m.solicitante)}</span>` : ""}
        ${m.encaminhado_por ? `<span><strong>Levado por:</strong> ${escapeHtml(m.encaminhado_por)}</span>` : ""}
        ${m.recebido_manutencao_por ? `<span><strong>Recebido na manutenção por:</strong> ${escapeHtml(m.recebido_manutencao_por)}</span>` : ""}
        ${m.responsavel_manutencao ? `<span><strong>Responsável pela manutenção:</strong> ${escapeHtml(m.responsavel_manutencao)}</span>` : ""}
        ${m.entregue_centro_por ? `<span><strong>Entregue ao centro por:</strong> ${escapeHtml(m.entregue_centro_por)}</span>` : ""}
        ${m.recebido_centro_por ? `<span><strong>Recebido no centro por:</strong> ${escapeHtml(m.recebido_centro_por)}</span>` : ""}
      </div>
      <small>Início: ${formatDate(m.data_inicio)}${m.data_fim ? ` · Conclusão: ${formatDate(m.data_fim)}` : ""}</small>
      ${m.status === "ABERTA" && canCreate ? `<button class="ghost-button compact-button" data-action="begin-finish-maintenance" data-maintenance-id="${m.id}">Concluir manutenção</button>` : ""}
      ${state.maintenanceCompletingId === m.id ? `<form class="finish-maintenance-form" data-maintenance-finish-id="${m.id}">
        <div class="operation-layout compact-operation-layout">
          ...
          <section class="operation-section">
            ...
            <label class="full">Observação da conclusão<textarea name="observacao_conclusao" rows="3"></textarea></label>
          </section>
        </div>
    ...
    <div class="operation-layout">
      ...
      <section class="operation-section">
        ...
        <div class="form-grid operation-people-grid">
          ...
          <label>Levado para manutenção por *<input name="encaminhado_por" required maxlength="120"></label>
          <label>Recebido na manutenção por *<input name="recebido_manutencao_por" required maxlength="120"></label>
        </div>
      </section>
      ...
      <section class="operation-section">
        ...
        <label class="full">Descrição *<textarea name="descricao" rows="4" required placeholder="Descreva a manutenção"></textarea></label>
      </section>
    </div>
